...
(defvar finder-inf-content nil
  "String holding the generated finder-inf.el content.
Set by `finder-compile-keywords'.  Diagnostic 'Finder running:' beacons mark
each step so the CI log can pinpoint where the 2-vCPU windows-latest runner
wedges after 'Scanning files for finder...done'.")
  ...
  (setq package--builtins
	(sort package--builtins
	      (lambda (a b) (string< (symbol-name (car a))
				     (symbol-name (car b))))))
  (byte-compile-info "Finder running: sorted builtins")

  ;; Build the generated content and write the file.  The write-region step
  ;; (mirroring loaddefs-generate) is the historical path; the "Finder
  ;; running:" beacons that precede it let the CI log pinpoint exactly where
  ;; the 2-vCPU windows-latest runner wedges (after 'Scanning files for
  ;; finder...done').
  (setq finder-inf-content
        (with-temp-buffer
          (generate-lisp-file-heading
           generated-finder-keywords-file 'finder-compile-keywords
           :title "keyword-to-package mapping")
          (byte-compile-info "Finder running: heading inserted")
          ;; FIXME: Now that we have package--builtin-versions,
          ;; package--builtins is only needed to get the list of unversioned
          ;; packages and to get the summary description of each package.
          ...
          (dolist (package package--builtins)
            ...
            (insert "\n"))
          (insert "))\n\n")
          (byte-compile-info "Finder running: builtins printed")
          ;; Insert hash table.
          (insert "(setq finder-keywords-hash\n      ")
          (prin1 finder-keywords-hash (current-buffer))
          (insert ")\n")
          (byte-compile-info "Finder running: hash printed")
          (generate-lisp-file-trailer generated-finder-keywords-file)
          (buffer-string)))
  (byte-compile-info "Finder running: content built")
  (with-temp-buffer
    (insert finder-inf-content)
    (write-region (point-min) (point-max)
                  generated-finder-keywords-file nil 'silent))
  (byte-compile-info "Finder running: file written"))
