  ...
  (let* ((event (json-parse-string value :object-type 'plist))
         (logical (decode-coding-string
                   (base64-decode-string
                    (plist-get event :logical-key)) 'utf-8))
         (state (plist-get event :state))
         (modifiers (plist-get event :modifiers))
         (physical (plist-get event :physical-key)))
    (when (and (= state 1) (= modifiers 2))
      (with-current-buffer (window-buffer (selected-window))
        (cond ((and (= physical 4) (string= logical "a"))
               (beginning-of-line))
              ((and (= physical 8) (string= logical "e"))
               (end-of-line)))
        (set-window-point (selected-window) (point))
        (redisplay)))))
