...
fn runEmacsInteractive(gpa: std.mem.Allocator, io: std.Io, config: *const Config) !void {
    ...
    const eval = try std.fmt.allocPrint(gpa,
        ...
        \\         (window-facts (proto-ui-window-facts frame))
        \\         (windows (plist-get (json-parse-string window-facts :object-type (quote plist) :array-type (quote vector) :false-object nil :null-object nil) :windows))
        ...
        \\      (setq window-facts (proto-ui-window-facts frame)) (setq windows (plist-get (json-parse-string window-facts :object-type (quote plist) :array-type (quote vector) :false-object nil :null-object nil) :windows)) (setq facts (json-parse-string (proto-ui-frame-facts frame) :object-type (quote plist)))
        \\      (let ((coding-system-for-write (quote utf-8))) (with-temp-file path (insert (json-serialize (list :frame_width (plist-get facts :frame_width) :frame_height (plist-get facts :frame_height) :window_width (plist-get facts :window_width) :window_height (plist-get facts :window_height) :windows windows :text (vconcat lines) :window_start_line viewport-start-line :window_visible_lines viewport-line-count :cursor cursor)))))
        \\      (sit-for 0.05)))))))
...
fn runFactsPublisher(gpa: std.mem.Allocator, io: std.Io, config: *Config) !void {
    ...
    for (0..@max(1, config.resync_sessions)) |session_index| {
        ...
        var acks = live.AckTracker.init(1);
        try sendSnapshotMessages(gpa, published.?, published.?.text.lines, published.?.cursor, published.?.viewport, &scene, &acks, io, &reader, &writer, input_path, &input_sequence, negotiated.effective);
        ...
        while (waited_ms < publish_duration) {
            ...
            if (heartbeat_due and (!config.interactive_publisher or heartbeat_ms >= 100)) {
                ...
                sendSnapshotMessages(gpa, published.?, published.?.text.lines, published.?.cursor, published.?.viewport, &scene, &change_acks, io, &reader, &writer, input_path, &input_sequence, negotiated.effective) catch |err| {
...
fn sendSnapshotMessages(
    ...
    try facts.appendWireSnapshotWindows(gpa, snapshot.facts, snapshot.windows, text, cursor, viewport, scene, &messages);
...
fn runRuntimeBridgeSmoke(gpa: std.mem.Allocator, config: *const Config) !void {
    ...
    for (draw_list.commands.items) |command| {
        switch (command) {
            .fill => |fill| {
                if (fill.rect.x == 8 and fill.rect.y == 8 and
                    fill.rect.width == 64 and fill.rect.height == 16 and
                    fill.color.r == 0x20 and fill.color.g == 0x24 and fill.color.b == 0x2c)
                    menu_box_rendered = true;
            },
            .text => |text| {
                if (text.x == 12 and text.y == 11 and std.mem.eql(u8, text.bytes, "File"))
        ...
        if (scene.windows.items.len != 0) {
            ...
            for (model.nodes) |*node| {
                ...
                if (input_policy.isAsciiText(label)) {
                    try list.drawText(rect.x + 4, rect.y + 3, label, .{ .r = 0xff, .g = 0xd5, .b = 0x4d });
                }
                offset += 64;
            }
        }
