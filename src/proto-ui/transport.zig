...
pub fn readReplay(gpa: std.mem.Allocator, io: std.Io, path: []const u8) ![][]const u8 {
    ...
    if (offset != data.len) return protocol.Error.TrailingBytes;
    return messages;
}
...
test "replay file rejects bad count short length and oversized input" {
    ...
    const trailing_path = try std.fmt.allocPrint(a, "{s}trailing", .{prefix});
    defer a.free(trailing_path);
    try tmp.dir.writeFile(io, .{ .sub_path = "trailing", .data = &trailing });
    try std.testing.expectError(protocol.Error.TrailingBytes, readReplay(a, io, trailing_path));
