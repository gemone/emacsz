...
pub const integration_points = [_]IntegrationPoint{
    ...
    .{ .id = "emacs_interactive_bridge", .owner = .frontend, .status = .partial, .summary = "real SDL facts window with atomic bounded local actions and an opt-in authenticated EPXL interactive input smoke; redisplay-owned glyphs and full interactive input pending" },
