...
pub const integration_points = [_]IntegrationPoint{
    ...
    .{ .id = "frame_pacing", .owner = .frontend, .status = .partial, .summary = "dirty/resize-aware full-frame gate with present/skip, frame-path, and initial/cursor/viewport/unchanged damage counters; cursor-only changes use a bounded retained-frame clip; text/viewport changes and cursor fallback remain full-frame, while general clipped rendering/GPU timestamps pending" },
