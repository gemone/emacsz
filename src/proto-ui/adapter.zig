...
pub const integration_points = [_]IntegrationPoint{
    ...
    .{ .id = "input_translation", .owner = .frontend, .status = .partial, .summary = "bounded SDL key/text translation queue for printable ASCII plus backspace/cursor/copy and best-effort pointer motion plus ordered left press/drag/release sessions through the persistent EPXL delivery journal; modifiers, Unicode, focus, full keymaps, selection drag, wheel, and related capabilities pending" },
