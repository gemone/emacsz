...
pub const integration_points = [_]IntegrationPoint{
    ...
    .{ .id = "frame_resource_contract", .owner = .adapter, .status = .partial, .summary = "bounded frame create/update/destroy state machine and resource-generation declaration contract implemented; real frame teardown, resource payloads, deletion transport, snapshots, eviction, and full resource parity pending" },
