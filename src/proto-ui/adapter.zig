...
pub const integration_points = [_]IntegrationPoint{
    ...
    .{ .id = "frame_resource_contract", .owner = .adapter, .status = .partial, .summary = "bounded frame create/update/destroy state machine, real-frame lifecycle smoke, visibility/focus contract, and resource-generation declaration contract implemented; output_proto frame ownership, redisplay integration, resource payloads, deletion transport, snapshots, eviction, and full resource parity pending" },
