...
pub const integration_points = [_]IntegrationPoint{
    ...
    .{ .id = "frame_pacing", .owner = .frontend, .status = .partial, .summary = "dirty/resize-aware full-frame gate with present/skip, frame-path, and initial/cursor/viewport/unchanged damage counters; conservative viewport changes still redraw full-frame and clipped rendering/GPU timestamps pending" },
