...
/* Emacs omits gnulib's fcntl module on Windows (see nt/gnulib-cfg.mk):
   w32.c implements fcntl and nt/inc/sys/socket.h defines the F_* and
   O_NONBLOCK macros.  Declare the w32 fcntl and the O_* fallbacks the
   gnulib body would otherwise supply, using the same values as
   nt/inc/sys/socket.h so the two headers can be included in any order.  */
int fcntl (int fd, int action, int options);
# ifndef O_NONBLOCK
