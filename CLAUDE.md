...
- **Goal 1 — Build entirely with `zig build`, no `automake`/`make`: ✅ Done.**
  `zig build` is self-sufficient and shell-free: `src/config.h` is
  generated by a pure-Zig tool (`tools/gen-config` from the committed
  `src/config.h.in` template + `src/config_values.txt`, with full knob
  parity against the autoconf output), and the dump/loaddefs/compile/
