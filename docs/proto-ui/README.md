...
| Selection state | Negotiated bounded primary owner set/clear transitions transport into Scene, and bounded UTF-8 text round-trips through the negotiated SDL PRIMARY API (OS-shared on X11/Wayland; app-local SDL fallback elsewhere) | No platform selection ownership, target negotiation, multi-format data, or general Emacs selection parity |
