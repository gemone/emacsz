...
| Window state intents | Negotiated SDL resize/move/maximize intents apply through public Emacs frame APIs; maximize evidence records parameter acceptance, with actual completion host/WM-dependent | Exact WM geometry/completion, close/fullscreen/minimize/restore runtime, and multi-frame control remain pending |
| Selection state | Bounded Scene state validates primary ownership/request/transfer records, and bounded UTF-8 text round-trips through the negotiated SDL PRIMARY API (OS-shared on X11/Wayland; app-local SDL fallback elsewhere) | No platform selection ownership, target negotiation, multi-format data, or general Emacs selection parity |
