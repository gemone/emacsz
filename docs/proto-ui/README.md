...
| Window geometry intents | Negotiated SDL resize/move intents apply through public Emacs `set-frame-size`/`set-frame-position` with deterministic publisher acknowledgement | Exact WM geometry, close/fullscreen/maximize/minimize/restore runtime, and multi-frame control remain pending |
