...
| Selection state | Negotiated primary owner set/clear, bounded request/data completion, and matched error transport into Scene; bounded UTF-8 text also round-trips through the negotiated SDL PRIMARY API (OS-shared on X11/Wayland; app-local SDL fallback elsewhere) | No platform selection ownership, target conversion, multi-format data, or general Emacs selection parity |
