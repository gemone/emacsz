...
P57 real-window snapshot preparation projects bounded public live-window geometry and per-window point cursors into `FRAME_UPDATE`; the selected window owns the active EUP cursor and SDL receives other observed windows as authoritative boxes.  Cross-restart stable IDs, redisplay-owned cursor semantics, hierarchy, faces, and command dispatch remain pending; bounded per-window text and mode-line observations are now unit-projected.
