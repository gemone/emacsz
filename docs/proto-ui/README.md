...
| Window state intents | Negotiated SDL resize/move/fullscreen/maximize intents apply through or record public Emacs frame-parameter acceptance | Exact WM geometry/completion, close/minimize/restore runtime, and multi-frame control remain pending |
| Selection state | Negotiated primary owner set/clear and bounded request/data completion transport into Scene; bounded UTF-8 text also round-trips through the negotiated SDL PRIMARY API (OS-shared on X11/Wayland; app-local SDL fallback elsewhere) | No platform selection ownership, target conversion, error-path runtime, multi-format data, or general Emacs selection parity |
