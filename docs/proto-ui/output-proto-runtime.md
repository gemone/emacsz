...
| `proto-ui-protocol-coverage` step | Audit every assigned EUP message ID against its implementation status | Implemented; 47 codecs implemented, 3 partial, and 114 planned |
| `session` module | Standard EUP setup/control codecs, setup state machine, Scene control integration, automatic frontend PONG, and EPXL suspend/resume/liveness/error/close evidence | Implemented as bounded protocol preparation; VERSION_MISMATCH transport and full runtime ownership remain pending |
