...
| `session` module | Standard EUP setup/control codecs, setup state machine, Scene control integration, and EPXL suspend/resume/liveness/error/close evidence | Implemented as bounded protocol preparation; automatic PONG, VERSION_MISMATCH transport, and full runtime ownership remain pending |
