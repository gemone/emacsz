...
| Terminal deletion | PGTK terminal hooks | EUP session/frame teardown | P0 | Degraded | TPE headless cleanup plus adapter-owned frame/terminal hooks stop the active SDL surface, remove its wait descriptor, drain/destroy the adapter session, unregister the provider, and delete the terminal (`proto-ui-tpe-frame-cleanup`); frontend EOF takes the same inherited terminal-deletion rollback (`proto-ui-tpe-frame-eof`), while reconnect/resynchronization remains pending |
...
| Capability | Priority | Status | Evidence |
|---|---|---|---|
| Keyboard events | P0 | Degraded | Printable ASCII insert, backspace, arrows, copy, paste; TPE provider uses SDL layout keycodes for printable input and scancode mapping for navigation/function keys, while `F1-F12`, `Home`, `End`, `PageUp/PageDown`, `Insert`, and `Delete` feed standard Emacs key symbols and auto-repeat reaches the inherited key queue (`proto-ui-tpe-input`); negotiated single-key and exact `C-x 1/2/3/o` commands map a closed SDL whitelist to bounded Emacs key descriptions; smokes observe `C-x 2`, `C-x 3` plus `C-x o` navigation/edit, and `C-x 1` one-window restore; no general keymap/IME/session parity |
| Modifier state | P0 | Degraded | Ctrl+C/V plus bounded C/M/s/H-modified key descriptions and exact two-key `C-x 1/2/3/o` commands through full key v2 with `input.key_command_v1` and `input.composite_key_command_v1`; the TPE provider maps Shift/Ctrl/Alt/Super/Mode to standard Emacs modifiers, round-trips each plus their combined form on `F12`, and proves printable `a`, `A`, `C-c`, `A-c`, `s-c`, and `M-c` (`proto-ui-tpe-input`); arbitrary modifier combinations, complete Emacs translation, and general command parity pending |
...
| Wheel scroll | P0 | Degraded | Bounded whole-line vertical and horizontal ticks; TPE carries signed high-resolution X/Y centi-unit deltas in the standard wheel-event `arg`, and `pixel-scroll-precision` consumes them to change the provider window scroll state (`proto-ui-tpe-input`); provider wheel events expose standard `mouse` device class, while momentum, page units, and smooth scrolling pending |
| Touchpad scroll | P1 | Pending | W12/W16 PGTK parity gate not met |
| Touch | EXP | Degraded | Negotiated `input.touch_bounded_v1` converts one SDL finger contact into a bounded Pointer v2 press/drag/release/cancel (`sdl3-touch-tap-smoke`); the TPE provider path maps native SDL finger down/motion/up/cancel to standard touchscreen begin/update/end with active-ID tracking, bounded multi-touch `touchscreen-update` aggregation for up to eight simultaneous contacts, and standard `touchscreen` device class (`proto-ui-tpe-input`); gestures, pressure, pen, and Emacs gesture commands remain pending |
