...
| Capability | Priority | Status | Evidence |
|---|---|---|---|
| Keyboard events | P0 | Degraded | Printable ASCII insert, backspace, arrows, copy, paste; TPE provider navigation/function keys `F1-F12`, `Home`, `End`, `PageUp/PageDown`, `Insert`, and `Delete` feed standard Emacs key symbols (`proto-ui-tpe-input`); negotiated single-key and exact `C-x 1/2/3/o` commands map a closed SDL whitelist to bounded Emacs key descriptions; smokes observe `C-x 2`, `C-x 3` plus `C-x o` navigation/edit, and `C-x 1` one-window restore; no general keymap/IME/session parity |
