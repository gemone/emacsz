...
| `input.pen_bounded_v1` | Optional | adapter/frontend | Requires negotiated `input.pointer_v2`; the TPE provider path maps a valid pen tip and its first two barrel buttons to standard mouse/drag/button events with `pen` device class, while the eraser tip, remaining barrel buttons, pressure axes, tilt, and proximity events stay unclaimed |
