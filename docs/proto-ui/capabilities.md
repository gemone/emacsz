...
| Touch | EXP | Degraded | Negotiated `input.touch_bounded_v1` converts one SDL finger contact into a bounded Pointer v2 press/drag/release/cancel (`sdl3-touch-tap-smoke`); the TPE provider path maps native SDL finger-up to standard Emacs `touch-end` (`proto-ui-tpe-input`); multi-touch, gestures, pressure, pen, and Emacs gesture commands remain pending |
