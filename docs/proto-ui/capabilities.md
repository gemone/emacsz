...
| Emacs observation | Real Emacs process publishes public frame geometry, bounded selection-relative flat live-window geometry (up to 16 windows, process-lifetime nonzero weak-key IDs that are not reused), and bounded UTF-8 `TEXT_LINE_V2` for up to eight visible lines per window through `FRAME_UPDATE`; live-publisher multi-window content is currently unit-projected rather than split-smoke tested; only the selected window has a published cursor; SDL debug rendering is ASCII-only; W12c creates/deletes one real display-backed frame and synchronizes one EUP/SDL3 frame lifecycle; W10e renders that public-facts marker through the bounded glyph-run debug fallback | Per-window cursors, more than eight visible lines per window, cross-restart window identity, hierarchical redisplay-owned window trees, rows/glyphs/faces/fonts, `output_proto`-owned frame creation/deletion, runtime visibility/focus events | `proto-ui-module-smoke`, `proto-ui-frame-fact-smoke`, `sdl3-emacs-smoke`, `sdl3-epxl-facts-smoke`, `sdl3-frame-smoke` |
