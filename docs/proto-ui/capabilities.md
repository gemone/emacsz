...
|---|---|---|---|
| Protocol coverage | All 164 assigned EUP IDs are classified in a deterministic manifest: 144 implemented codecs, 3 partial, and 17 planned; no unassigned or unclassified ID | Production implementation of the remaining face/text/graphics protocol gaps |
| Protocol/transport | EUP envelope, bounded `FRAME_UPDATE`, replay, EPXL framing, resync, ACK/retry, bounded in-session display sequence-gap recovery, bounded selection/clipboard transfer codecs with explicit clipboard-kind wrappers, deterministic ordered/resync/ACK-loss/ERP1 convergence differential with
