...
| Window requests | P1 | Degraded | Negotiated close/resize/move/fullscreen/maximize/minimize/restore intents; bounded resize/move apply through public `set-frame-size`/`set-frame-position`, and bounded maximize records Emacs parameter acceptance (`sdl3-window-resize-roundtrip-smoke`, `sdl3-window-move-roundtrip-smoke`, `sdl3-window-maximize-roundtrip-smoke`); actual maximize completion, other requests, host contract, exact WM geometry, and multi-frame parity remain pending |
