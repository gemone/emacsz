...
| Capability | Priority | Status | Evidence |
|---|---|---|---|
| Keyboard events | P0 | Degraded | Printable ASCII insert, backspace, arrows, copy, paste; negotiated `input.key_command_v1` maps a closed SDL whitelist to bounded Emacs key descriptions and Emacs executes those descriptions; no general keymap/IME/session parity |
| Modifier state | P0 | Degraded | Ctrl+C/V plus bounded C/M/s/H-modified key descriptions through full key v2 and `input.key_command_v1`; arbitrary modifier combinations, complete Emacs translation, and general command parity pending |
