...
| `input.pen_bounded_v1` | Optional | adapter/frontend | Requires negotiated `input.pointer_v2`; the TPE provider path maps a valid pen tip and all five SDL barrel buttons to standard mouse/drag/button events with `pen` device class, exposes pressure and X/Y tilt as bounded read-only adapter metadata without inventing core input semantics, and leaves the eraser tip, remaining axes, proximity events, and drawing surfaces unclaimed |
