...
| Modifier state | P0 | Degraded | Ctrl+C/V plus bounded C/M/s/H-modified key descriptions and exact two-key `C-x 1/2/3/o` commands through full key v2 with `input.key_command_v1` and `input.composite_key_command_v1`; the TPE provider maps Shift/Ctrl/Alt/Super/Mode to standard Emacs modifiers and round-trips each plus their combined form on `F12` (`proto-ui-tpe-input`); arbitrary modifier combinations, complete Emacs translation, and general command parity pending |
