...
5. **Resource model.** Generation declarations, a bounded payload cache/eviction policy, request/evict wire contracts, bounded `STRING_DEFINE`/`STRING_DELETE`, and bounded fixed-layout `FACE_DEFINE`/`FACE_DELETE` plus `FONT_DEFINE`/`FONT_DELETE` v1 exist. Actual image payload definitions, snapshots, recovery, rendering, redisplay face/font capture, and full resource parity remain pending.
