...
| Emacs observation | Real Emacs process publishes public frame geometry, bounded selection-relative flat live-window geometry (up to 16 windows, process-lifetime nonzero weak-key IDs that are not reused) through `FRAME_UPDATE`, bounded printable-ASCII text, point/cursor, and viewport facts for the selected window; W12c creates/deletes one real display-backed frame and synchronizes one EUP/SDL3 frame lifecycle; W10e renders that public-facts marker through the bounded glyph-run debug fallback | Cross-restart window identity, per-window rows/text/cursors, hierarchical redisplay-owned window trees, rows/glyphs/faces/fonts, `output_proto`-owned frame creation/deletion, runtime visibility/focus events | `proto-ui-module-smoke`, `proto-ui-frame-fact-smoke`, `sdl3-emacs-smoke`, `sdl3-epxl-facts-smoke`, `sdl3-frame-smoke` |
