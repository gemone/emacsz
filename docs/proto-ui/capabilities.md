...
| Wheel scroll | P0 | Degraded | Bounded whole-line vertical and horizontal ticks; TPE carries signed high-resolution X/Y centi-unit deltas in the standard wheel-event `arg`, and `pixel-scroll-precision` consumes them to change the provider window scroll state (`proto-ui-tpe-input`); momentum, page units, and smooth scrolling pending |
| Touchpad scroll | P1 | Pending | W12/W16 PGTK parity gate not met |
| Touch | EXP | Degraded | Negotiated `input.touch_bounded_v1` converts one SDL finger contact into a bounded Pointer v2 press/drag/release/cancel (`sdl3-touch-tap-smoke`); the TPE provider path maps native SDL finger down/motion/up/cancel to standard touchscreen begin/update/end with active-ID tracking (`proto-ui-tpe-input`); multi-touch aggregation, gestures, pressure, pen, and Emacs gesture commands remain pending |
