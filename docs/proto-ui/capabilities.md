...
| Touch | EXP | Degraded | Negotiated `input.touch_bounded_v1` converts one SDL finger contact into a bounded Pointer v2 press/drag/release/cancel (`sdl3-touch-tap-smoke`); the TPE provider path maps native SDL finger down/motion/up/cancel to standard touchscreen begin/update/end with active-ID tracking and standard `touchscreen` device class (`proto-ui-tpe-input`); multi-touch aggregation, gestures, pressure, pen, and Emacs gesture commands remain pending |
