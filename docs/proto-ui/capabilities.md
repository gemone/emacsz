...
W12h fail-closed runtime manifest/gate, and W12i generated C adapter plus
linkable observation library. 118 PGTK rows are audited: 21 Degraded,
...
|---|---|---|---|
| Protocol/transport | EUP envelope, bounded `FRAME_UPDATE`, replay, EPXL framing, resync, ACK/retry, bounded EPXL capability negotiation/status manifest, frame visibility/focus state codec, bounded resource payload cache/eviction policy, request/evict codecs, optional host frame-state ABI seam, terminal-lifecycle core, generated read-only C adapter, and dynamically linkable observation library | General resource/widget capability coverage, arbitrary recovery, remote safety, runtime terminal registration | `proto-ui-conformance`, `proto-ui-unit`, `proto-ui-shim-conformance`, `proto-ui-shim-library-conformance`, `sdl3-live-smoke`, `sdl3-epxl-resync-smoke`, `sdl3-epxl-recovery-smoke` |
...
5. **Resource model.** Generation declarations, a bounded payload cache/eviction policy, and request/evict wire contracts exist, but actual face/font/image payload definitions, deletion integration, snapshots, recovery, rendering, and full resource parity remain pending.

### 12.3 Minimum next milestone

With the R4 linkable observation library complete, the next adapter-first
milestone is R5 frame-service mapping and teardown before redisplay-owned rows,
