...
| P5-prep. Run/face payload bridge | Host face/font/image records become bounded EUP resources; host text runs become bounded debug glyph runs and are checked against live faces | `runtime_bridge` unit suite and `sdl3-runtime-bridge-smoke`; still not full redisplay capture or shaped text |
...
| P7-prep. Title bridge | EUP `FRAME_TITLE` resolves a live string resource and the diagnostic SDL bridge applies the Scene-owned title | `proto-ui-unit`, `sdl3-runtime-bridge-smoke`, and bounded authenticated Emacs title observation; output_proto-owned title ownership still requires R8 |
...
| P15-prep. Terminal runtime service | Host terminal create/activate/drain/delete callbacks drive a bounded no-reuse registry with drain retry and rollback-pending cleanup | `terminal_service` unit suite and `proto-ui-terminal-service`; real linked Emacs host and registration still required |
| P16-prep. Host adapter selection | A versioned pure-SDL3 `output_proto` candidate is selected by approved R7 policy with complete metadata and validation; native-glibc opt-in temacs linkage is audited as `linked_not_registered` | `host_adapter` unit suite, `proto-ui-host-adapter`, and `proto-ui-r8-link`; no call, runtime, or registration |
...
| P22-prep. Divider update | `DIVIDER_UPDATE` validates orientation, generation, window bounds, and renders a fixed-color divider | Draggable/resize semantics and redisplay ownership still require R8 |
| P23-prep. Fringe update | `FRINGE_UPDATE` validates left/right geometry, generation, and active window; SDL renders validated color bands | Bitmap patterns and redisplay-owned fringe semantics still require R8 |
| P4. Terminal registration | `output_proto` can exist as a real terminal without PGTK initialization | Live terminal lifecycle tests after explicit R8 adapter linkage and registration |
