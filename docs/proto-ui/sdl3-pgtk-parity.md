...
| P16-prep. Host adapter selection | A versioned pure-SDL3 `output_proto` candidate is selected by approved R7 policy with complete metadata and validation; real adapter linkage still requires R8 | `host_adapter` unit suite and `proto-ui-host-adapter`; runtime and registration remain absent |
