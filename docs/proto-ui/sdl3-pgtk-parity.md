...
| P5-prep. Run/face payload bridge | Host face records become EUP `FACE_DEFINE` resources; host text runs become bounded debug glyph runs and are checked against live faces | `runtime_bridge` unit suite and `sdl3-runtime-bridge-smoke`; still not full redisplay capture or shaped text |
