...
| Protocol coverage manifest | Implemented for all 164 assigned EUP IDs: 96 implemented codecs, 3 partial, and 65 planned; prevents an unclassified or overclaimed protocol table |
...
current counts are 96 implemented codecs, 3 partial, and 65 planned.
...
P49 frame-patch preparation adds `FRAME_PATCH` v1 as an exact 40-byte atomic batch for selected visibility, focus, opacity, decoration, and scale state.  The Scene validates active-frame identity and lifecycle consistency before applying any field.  SDL runtime smoke proves alpha/decoration/scale application.  Title, geometry, monitor, z-order, complete snapshot, and redisplay adaptation remain pending.
