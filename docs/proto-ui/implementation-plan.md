...
P58 multi-window content preparation publishes bounded `WINDOW_STATE` observations for every live window through the snapshot JSON and projects them as window-owned `TEXT_LINE_V2` rows.  A state carries `id`, `lines`, `window_start_line`, and `window_visible_lines`; publisher lines are UTF-8 byte-bounded to 120 bytes and capped to eight lines.  There is one state per live window in `window-list` order, and the top-level cursor remains the only EUP cursor.  Full per-window scroll, cursor, hierarchy, faces, and command semantics remain pending.
P57 real-window snapshot preparation projects bounded public `proto-ui-window-facts` into the `FRAME_UPDATE` window section.  The selected observed window carries the existing text/viewport/cursor rows; other observed windows become Scene windows so SDL can render their authoritative frame-relative geometry.  IDs are adapter-owned process-lifetime identities, not cross-restart identities; per-window cursors, buffers, rows, faces, hierarchy, and command dispatch remain pending.
