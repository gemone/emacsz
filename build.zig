...
pub fn build(b: *std.Build) void {
    ...
    if (enable_sdl3_frontend) {
        ...
        const sdl3_frontend = b.addExecutable(.{
            ...
            .root_module = b.createModule(.{
                .target = b.graph.host,
                .optimize = optimize,
                .root_source_file = b.path("tools/proto-ui-sdl3/main.zig"),
                .link_libc = true,
            }),
        });
    ...
    if (modules_runtime and enable_proto_ui) {
        ...
        const proto_module_smoke = b.addSystemCommand(&[_][]const u8{
            ...
            "--eval",
            "(let* ((frame (selected-frame)) (temp-a (split-window)) (raw-a (proto-ui-window-facts frame)) (facts-a (json-parse-string raw-a :object-type (quote plist) :array-type (quote list) :false-object nil :null-object nil)) (ids-a (sort (mapcar (lambda (window) (plist-get window :id)) (plist-get facts-a :windows)) #'<)) (_ (delete-window temp-a)) (temp-b (split-window)) (raw-b (proto-ui-window-facts frame)) (facts-b (json-parse-string raw-b :object-type (quote plist) :array-type (quote list) :false-object nil :null-object nil)) (ids-b (sort (mapcar (lambda (window) (plist-get window :id)) (plist-get facts-b :windows)) #'<))) (unless (and (= (length ids-a) 2) (= (length ids-b) 2) (< (apply #'max ids-a) (apply #'max ids-b)) (string= (plist-get facts-b :identity) \"process_lifetime\")) (error \"proto-ui stable-window-id smoke failed\")) (delete-window temp-b) (princ \"proto-ui-stable-window-id-smoke: pass\"))",
        });
        ...
        const run_sdl3_epxl_reconnect = b.addSystemCommand(&[_][]const u8{
            "./zig-out/bin/proto-ui-sdl3",
            "--emacs-epxl-reconnect-smoke",
            "--emacs",
            "./zig-out/bin/emacs",
            "--module",
            std.fmt.allocPrint(
        ...
        run_sdl3_epxl_reconnect.setCwd(b.path("."));
        run_sdl3_epxl_reconnect.step.dependOn(&proto_module_smoke.step);
        run_sdl3_epxl_reconnect.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |step| run_sdl3_epxl_reconnect.step.dependOn(step);
        const sdl3_epxl_reconnect_step = b.step(
            "sdl3-epxl-resync-smoke",
        ...
        const run_sdl3_epxl_recovery = b.addSystemCommand(&[_][]const u8{
            "./zig-out/bin/proto-ui-sdl3",
            "--emacs-epxl-recovery-smoke",
            "--emacs",
            "./zig-out/bin/emacs",
            "--module",
            std.fmt.allocPrint(
        ...
        run_sdl3_epxl_recovery.setCwd(b.path("."));
        run_sdl3_epxl_recovery.step.dependOn(&proto_module_smoke.step);
        run_sdl3_epxl_recovery.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |step| run_sdl3_epxl_recovery.step.dependOn(step);
        const sdl3_epxl_recovery_step = b.step(
            "sdl3-epxl-recovery-smoke",
            "Drop an input ACK, reconnect SDL3, and retry the same EPXL sequence",
        );
        sdl3_epxl_recovery_step.dependOn(&run_sdl3_epxl_recovery.step);

        const run_sdl3_epxl_gap = b.addSystemCommand(&[_][]const u8{
            "./zig-out/bin/proto-ui-sdl3",
            "--emacs-epxl-gap-recovery-smoke",
            "--emacs",
            "./zig-out/bin/emacs",
            "--module",
            std.fmt.allocPrint(
        ...
        run_sdl3_epxl_gap.setCwd(b.path("."));
        run_sdl3_epxl_gap.step.dependOn(&proto_module_smoke.step);
        run_sdl3_epxl_gap.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |step| run_sdl3_epxl_gap.step.dependOn(step);
        const sdl3_epxl_gap_step = b.step(
            "sdl3-epxl-gap-recovery-smoke",
            "Drop a display sequence and recover the real Emacs scene in-session",
        );
        sdl3_epxl_gap_step.dependOn(&run_sdl3_epxl_gap.step);
        ...
        run_sdl3_wheel.setCwd(b.path("."));
        run_sdl3_wheel.step.dependOn(&proto_module_smoke.step);
        run_sdl3_wheel.step.dependOn(b.getInstallStep());
        ...
        const run_sdl3_epxl_unicode_input = b.addSystemCommand(&[_][]const u8{
            "./zig-out/bin/proto-ui-sdl3",
            "--emacs-epxl-unicode-input-smoke",
            "--emacs",
            "./zig-out/bin/emacs",
            "--module",
            std.fmt.allocPrint(
        ...
        run_sdl3_epxl_unicode_input.setCwd(b.path("."));
        run_sdl3_epxl_unicode_input.step.dependOn(&proto_module_smoke.step);
        run_sdl3_epxl_unicode_input.step.dependOn(b.getInstallStep());
        ...
        run_sdl3_epxl_key_v2.setCwd(b.path("."));
        run_sdl3_epxl_key_v2.step.dependOn(&proto_module_smoke.step);
        run_sdl3_epxl_key_v2.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |step| run_sdl3_epxl_key_v2.step.dependOn(step);
        const sdl3_epxl_key_v2_step = b.step(
            "sdl3-epxl-key-v2-smoke",
            "Negotiate full key v2 and execute a bounded Emacs compatibility subset",
        );
        ...
        const run_sdl3_window_navigation = b.addSystemCommand(&[_][]const u8{
            "./zig-out/bin/proto-ui-sdl3",
            "--emacs-window-navigation-smoke",
            "--emacs",
            "./zig-out/bin/emacs",
            "--module",
            std.fmt.allocPrint(
        ...
        run_sdl3_window_navigation.setCwd(b.path("."));
        run_sdl3_window_navigation.step.dependOn(&proto_module_smoke.step);
        run_sdl3_window_navigation.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |step| run_sdl3_window_navigation.step.dependOn(step);
        const sdl3_window_navigation_step = b.step(
            "sdl3-emacs-window-navigation-smoke",
            "Execute C-x 3, select the other window, insert text, and verify both live windows",
        );
        ...
        run_sdl3_monitor_change.setCwd(b.path("."));
        run_sdl3_monitor_change.step.dependOn(&proto_module_smoke.step);
        run_sdl3_monitor_change.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |step| run_sdl3_monitor_change.step.dependOn(step);
        const sdl3_monitor_change_step = b.step(
            "sdl3-monitor-change-smoke",
            "Observe an SDL display change and refresh bounded monitor facts",
        );
        ...
        const run_sdl3_window_resize_roundtrip = b.addSystemCommand(&[_][]const u8{
            "./zig-out/bin/proto-ui-sdl3",
            "--emacs",
            "./zig-out/bin/emacs",
            "--module",
            ...
            "--emacs-window-resize-roundtrip-smoke",
            "--auto-quit-ms=5000",
        });
        run_sdl3_window_resize_roundtrip.setCwd(b.path("."));
        run_sdl3_window_resize_roundtrip.step.dependOn(&proto_module_smoke.step);
        run_sdl3_window_resize_roundtrip.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |step| run_sdl3_window_resize_roundtrip.step.dependOn(step);
        const sdl3_window_resize_roundtrip_step = b.step(
            "sdl3-window-resize-roundtrip-smoke",
            "Apply a bounded SDL resize intent through public Emacs APIs",
        );
        sdl3_window_resize_roundtrip_step.dependOn(&run_sdl3_window_resize_roundtrip.step);

        const run_sdl3_window_move_roundtrip = b.addSystemCommand(&[_][]const u8{
            "./zig-out/bin/proto-ui-sdl3",
            "--emacs",
            "./zig-out/bin/emacs",
            "--module",
            ...
            "--emacs-window-move-roundtrip-smoke",
            "--auto-quit-ms=5000",
        });
        run_sdl3_window_move_roundtrip.setCwd(b.path("."));
        run_sdl3_window_move_roundtrip.step.dependOn(&proto_module_smoke.step);
        run_sdl3_window_move_roundtrip.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |step| run_sdl3_window_move_roundtrip.step.dependOn(step);
        const sdl3_window_move_roundtrip_step = b.step(
            "sdl3-window-move-roundtrip-smoke",
            "Apply a bounded SDL move intent through public Emacs APIs",
        );
        sdl3_window_move_roundtrip_step.dependOn(&run_sdl3_window_move_roundtrip.step);

        const run_sdl3_window_maximize_roundtrip = b.addSystemCommand(&[_][]const u8{
            "./zig-out/bin/proto-ui-sdl3",
            "--emacs",
            "./zig-out/bin/emacs",
            "--module",
            ...
            "--emacs-window-maximize-roundtrip-smoke",
            "--auto-quit-ms=5000",
        });
        run_sdl3_window_maximize_roundtrip.setCwd(b.path("."));
        run_sdl3_window_maximize_roundtrip.step.dependOn(&proto_module_smoke.step);
        run_sdl3_window_maximize_roundtrip.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |step| run_sdl3_window_maximize_roundtrip.step.dependOn(step);
        const sdl3_window_maximize_roundtrip_step = b.step(
            "sdl3-window-maximize-roundtrip-smoke",
    ...
    if (proto_ui_runtime_adapter_target_lib) |adapter_lib| {
        ...
        const run_tpe_frame_smoke = b.addSystemCommand(&[_][]const u8{
            ...
            "--batch",
            "--eval",
            "(let* ((initial-frame (selected-frame)) (terms (terminal-list)) (terminal (car (last terms))) (frame (make-terminal-frame (list (cons 'terminal terminal)))) (second (condition-case nil (make-terminal-frame (list (cons 'terminal terminal))) (error nil)))) (unless (and (frame-live-p frame) (eq (window-system frame) 'proto) (numberp (frame-id frame)) (terminal-provider-frame-p frame) (null (terminal-provider-frame-p initial-frame))) (error \"proto-ui-tpe-frame failed: live=%S ws=%S id=%S provider=%S initial-provider=%S\" (frame-live-p frame) (window-system frame) (frame-id frame) (terminal-provider-frame-p frame) (terminal-provider-frame-p initial-frame))) (unless (and (null second) (frame-live-p frame)) (error \"proto-ui-tpe-frame failed: second-frame=%S\" second)) (select-frame frame) (unless (terminal-provider-frame-p) (error \"proto-ui-tpe-frame failed: selected-provider=%S\" (terminal-provider-frame-p))) (let ((frame-title-format \"Emacs Proto-UI P203\")) (redraw-frame frame) (unless (and (redisplay t) (terminal-provider-capture-p) (string= (frame-parameter frame \'name) \"Emacs Proto-UI P203\")) (error \"proto-ui-tpe-frame failed: title=%S capture=%S\" (frame-parameter frame \'name) (terminal-provider-capture-p))) (set-frame-parameter frame \'title \"Emacs Proto-UI P204 Explicit\") (redraw-frame frame) (unless (and (redisplay t) (terminal-provider-capture-p) (string= (terminal-provider-title) \"Emacs Proto-UI P204 Explicit\")) (error \"proto-ui-tpe-frame failed: explicit-title=%S\" (terminal-provider-title))) (set-frame-parameter frame \'title nil) (set-frame-name \"Emacs Proto-UI P204 Named\") (redraw-frame frame) (unless (and (redisplay t) (terminal-provider-capture-p) (string= (terminal-provider-title) \"Emacs Proto-UI P204 Named\") (string= (frame-parameter frame \'name) \"Emacs Proto-UI P204 Named\")) (error \"proto-ui-tpe-frame failed: named-title=%S name=%S\" (terminal-provider-title) (frame-parameter frame \'name)))) (sit-for 1) (unless (and (redisplay t) (terminal-provider-capture-p)) (error \"proto-ui-tpe-frame failed: owner capture after rejection\")) (princ \"proto-ui-tpe-frame: pass\\n\"))",
        });
        run_tpe_frame_smoke.setEnvironmentVariable("TPE_TITLE_SMOKE", "Emacs Proto-UI P204 Named");
        ...
        const run_tpe_frame_eof_smoke = b.addSystemCommand(&[_][]const u8{
            "./zig-out/bin/emacs",
            "--batch",
            "--eval",
            "(let* ((initial-frame (selected-frame)) (terms (terminal-list)) (terminal (car (last terms))) (frame (make-terminal-frame (list (cons 'terminal terminal))))) (unless (and (frame-live-p frame) (terminal-provider-frame-p frame)) (error \"proto-ui-tpe-frame-eof failed: attach=%S provider=%S\" (frame-live-p frame) (terminal-provider-frame-p frame))) (select-frame frame) (unless (redisplay t) (error \"proto-ui-tpe-frame-eof failed: display=%S\" (redisplay t))) (select-frame initial-frame) (read-event nil nil 2) (unless (and (not (frame-live-p frame)) (null (terminal-live-p terminal)) (= (length (terminal-list)) 1) (frame-live-p initial-frame)) (error \"proto-ui-tpe-frame-eof failed: frame=%S terminal=%S terms=%S initial=%S\" (frame-live-p frame) (terminal-live-p terminal) (terminal-list) (frame-live-p initial-frame))) (princ \"proto-ui-tpe-frame-eof: pass\\n\"))",
        });
        run_tpe_frame_eof_smoke.setEnvironmentVariable("TPE_EOF_SMOKE", "1");
        run_tpe_frame_eof_smoke.setEnvironmentVariable("EMACS_TERMINAL_PROVIDER", "proto");
        run_tpe_frame_eof_smoke.setCwd(b.path("."));
        run_tpe_frame_eof_smoke.step.dependOn(b.getInstallStep());
        if (sdl3_frontend_dep) |dep| run_tpe_frame_eof_smoke.step.dependOn(dep);
        const tpe_frame_eof_step = b.step(
            "proto-ui-tpe-frame-eof",
            "Recover the original frame after provider frontend EOF",
        );
        ...
        run_tpe_frame_visibility_smoke.addFileArg(b.path("test/proto-ui/tpe-frame-visibility.el"));
        run_tpe_frame_visibility_smoke.addArgs(&[_][]const u8{
            "--eval",
        ...
        const tpe_frame_visibility_step = b.step(
            "proto-ui-tpe-frame-visibility",
            "Observe SDL minimization and restoration as Emacs frame visibility",
        );
        ...
        const run_tpe_input_smoke = b.addSystemCommand(&[_][]const u8{
            ...
            "--batch",
            "--eval",
            "(let* ((initial-frame (selected-frame)) (terms (terminal-list)) (terminal (car (last terms))) (frame (make-terminal-frame (list (cons 'terminal terminal)))) (window-states nil) (old-iconify (lookup-key special-event-map [iconify-frame])) (old-restore (lookup-key special-event-map [make-frame-visible])) (maximize-states nil) (maximize-timer (run-with-timer 0.05 0.05 (lambda () (push (frame-parameter frame 'fullscreen) maximize-states)))) (close-events nil) (old-close (lookup-key special-event-map [delete-frame]))) (setq frame-notice-user-settings nil) (unless (and (frame-live-p frame) (eq (window-system frame) 'proto)) (error \"proto-ui-tpe-input failed: frame\")) (select-frame frame) (define-key special-event-map [iconify-frame] (lambda () (interactive) (push 'iconify-frame window-states))) (define-key special-event-map [make-frame-visible] (lambda () (interactive) (push 'make-frame-visible window-states))) (define-key special-event-map [delete-frame] (lambda (event) (interactive \"e\") (push event close-events))) (erase-buffer) (insert \"alpha\nbeta\ngamma\n\") (let ((overlay (make-overlay 1 18))) (overlay-put overlay 'mouse-face 'highlight)) (unless (and (redisplay t) (terminal-provider-capture-p)) (error \"proto-ui-tpe-input failed: capture\")) (let ((keys nil) (deadline (+ (float-time) 2))) (while (and (< (length keys) 22) (< (float-time) deadline)) (let ((key (read-event nil nil 0.5))) (when key (push key keys)))) (setq keys (vconcat (nreverse keys))) (unless (and (= (length keys) 22) (equal (mapcar #'single-key-description (substring keys 0 6)) '(\"a\" \"A\" \"C-c\" \"A-c\" \"s-c\" \"M-c\")) (equal (substring keys 6 16) [32 left left f12 delete home prior next end insert]) (equal (mapcar #'single-key-description (substring keys 16)) '(\"C-<f12>\" \"A-<f12>\" \"s-<f12>\" \"M-<f12>\" \"S-<f12>\" \"A-C-M-S-s-<f12>\")) (eq (device-class frame last-event-device) 'keyboard)) (error \"proto-ui-tpe-input failed: keys=%S descriptions=%S device=%S\" keys (mapcar #'single-key-description keys) last-event-device))) (let ((down (read-event nil nil 2))) (unless (eq (car-safe down) 'down-mouse-1) (error \"proto-ui-tpe-input failed: down=%S\" down))) (let ((up (read-event nil nil 2))) (unless (eq (car-safe up) 'mouse-1) (error \"proto-ui-tpe-input failed: up=%S\" up))) (let ((double-down (read-event nil nil 2)) (double-up (read-event nil nil 2))) (unless (and (eq (car-safe double-down) 'double-down-mouse-1) (= (event-click-count double-down) 2) (eq (car-safe double-up) 'double-mouse-1) (= (event-click-count double-up) 2)) (error \"proto-ui-tpe-input failed: double=%S %S\" double-down double-up))) (let ((down2 (read-event nil nil 2))) (unless (eq (car-safe down2) 'down-mouse-2) (error \"proto-ui-tpe-input failed: down2=%S\" down2))) (let ((up2 (read-event nil nil 2))) (unless (eq (car-safe up2) 'mouse-2) (error \"proto-ui-tpe-input failed: up2=%S\" up2))) (let ((down3 (read-event nil nil 2))) (unless (eq (car-safe down3) 'down-mouse-3) (error \"proto-ui-tpe-input failed: down3=%S\" down3))) (let ((up3 (read-event nil nil 2))) (unless (eq (car-safe up3) 'mouse-3) (error \"proto-ui-tpe-input failed: up3=%S\" up3))) (let ((down4 (read-event nil nil 2))) (unless (eq (car-safe down4) 'down-mouse-4) (error \"proto-ui-tpe-input failed: down4=%S\" down4))) (let ((up4 (read-event nil nil 2))) (unless (eq (car-safe up4) 'mouse-4) (error \"proto-ui-tpe-input failed: up4=%S\" up4))) (let ((down5 (read-event nil nil 2))) (unless (eq (car-safe down5) 'down-mouse-5) (error \"proto-ui-tpe-input failed: down5=%S\" down5))) (let ((up5 (read-event nil nil 2))) (unless (eq (car-safe up5) 'mouse-5) (error \"proto-ui-tpe-input failed: up5=%S\" up5))) (let ((wheel (read-event nil nil 2))) (unless (eq (car-safe wheel) 'wheel-down) (error \"proto-ui-tpe-input failed: wheel=%S\" wheel))) (let ((left (read-event nil nil 2))) (unless (eq (car-safe left) 'wheel-left) (error \"proto-ui-tpe-input failed: left=%S\" left))) (let ((right (read-event nil nil 2))) (unless (eq (car-safe right) 'wheel-right) (error \"proto-ui-tpe-input failed: right=%S\" right))) (let ((high-res (read-event nil nil 2))) (unless (and (eq (car-safe high-res) 'wheel-up) (equal (nth 4 high-res) '(0.0 . 50.0))) (error \"proto-ui-tpe-input failed: high-res=%S\" high-res))) (let ((high-res-down (read-event nil nil 2))) (unless (and (eq (car-safe high-res-down) 'wheel-down) (equal (nth 4 high-res-down) '(0.0 . -50.0))) (error \"proto-ui-tpe-input failed: high-res-down=%S\" high-res-down)) (insert (make-string 120 ?\\n)) (goto-char (point-min)) (redisplay t) (let ((scroll-start (window-start)) (scroll-vscroll (window-vscroll nil t))) (require 'term/proto-win) (require 'pixel-scroll) (unless (eq (device-class frame last-event-device) 'mouse) (error \"proto-ui-tpe-input failed: wheel-device=%S\" last-event-device)) (let ((pixel-scroll-precision-interpolate-mice nil)) (pixel-scroll-precision high-res-down) (unless (or (/= (window-start) scroll-start) (/= (window-vscroll nil t) scroll-vscroll)) (error \"proto-ui-tpe-input failed: pixel-scroll=%S start=%S vscroll=%S\" (window-start) scroll-start (window-vscroll nil t)))))) (let* ((touch-begin (read-event nil nil 2)) (touch-begin2 (read-event nil nil 2)) (touch-update (read-event nil nil 2)) (touch-end (read-event nil nil 2)) (touch-end2 (read-event nil nil 2))) (unless (and (eq (car-safe touch-begin) 'touchscreen-begin) (eq (car-safe (nth 1 touch-begin)) 7) (windowp (car-safe (cdr-safe (nth 1 touch-begin))))) (error \"proto-ui-tpe-input failed: touch-begin=%S\" touch-begin)) (unless (and (eq (car-safe touch-begin2) 'touchscreen-begin) (eq (car-safe (nth 1 touch-begin2)) 8) (windowp (car-safe (cdr-safe (nth 1 touch-begin2))))) (error \"proto-ui-tpe-input failed: touch-begin2=%S\" touch-begin2)) (unless (and (eq (car-safe touch-update) 'touchscreen-update) (equal (mapcar #'car (nth 1 touch-update)) '(8 7))) (error \"proto-ui-tpe-input failed: touch-update=%S\" touch-update)) (unless (and (eq (car-safe touch-end) 'touchscreen-end) (eq (car-safe (nth 1 touch-end)) 7) (null (nth 2 touch-end)) (eq (device-class frame last-event-device) 'touchscreen)) (error \"proto-ui-tpe-input failed: touch-end=%S device=%S\" touch-end last-event-device)) (unless (and (eq (car-safe touch-end2) 'touchscreen-end) (eq (car-safe (nth 1 touch-end2)) 8) (null (nth 2 touch-end2))) (error \"proto-ui-tpe-input failed: touch-end2=%S\" touch-end2))) (let ((pen-down (read-event nil nil 2)) (pen-drag (read-event nil nil 2))) (unless (and (eq (car-safe pen-down) 'down-mouse-1) (eq (device-class frame last-event-device) 'pen) (eq (car-safe pen-drag) 'drag-mouse-1) (eq (device-class frame last-event-device) 'pen)) (error \"proto-ui-tpe-input failed: pen=%S %S device=%S\" pen-down pen-drag last-event-device))) (dolist (pen-barrel '(mouse-2 mouse-3 mouse-4 mouse-5 mouse-6)) (let ((pen-barrel-down (read-event nil nil 2)) (pen-barrel-up (read-event nil nil 2))) (unless (and (eq (car-safe pen-barrel-down) (intern (format \"down-%s\" pen-barrel))) (eq (device-class frame last-event-device) 'pen) (eq (car-safe pen-barrel-up) pen-barrel) (eq (device-class frame last-event-device) 'pen)) (error \"proto-ui-tpe-input failed: pen-barrel=%S %S expected=%S device=%S\" pen-barrel-down pen-barrel-up pen-barrel last-event-device)))) (unless (equal (terminal-provider-pen-proximity) '(out 1 1)) (error \"proto-ui-tpe-input failed: pen-proximity=%S\" (terminal-provider-pen-proximity))) (unless (and (= (terminal-provider-pen-axis 0) 0.625) (= (terminal-provider-pen-axis 1) -45.0) (= (terminal-provider-pen-axis 2) 30.0) (= (terminal-provider-pen-axis 3) 0.25) (= (terminal-provider-pen-axis 4) -120.0) (= (terminal-provider-pen-axis 5) 0.75) (= (terminal-provider-pen-axis 6) -0.5) (null (terminal-provider-pen-axis 7))) (error \"proto-ui-tpe-input failed: pen-axis=%S %S %S %S %S %S %S %S\" (terminal-provider-pen-axis 0) (terminal-provider-pen-axis 1) (terminal-provider-pen-axis 2) (terminal-provider-pen-axis 3) (terminal-provider-pen-axis 4) (terminal-provider-pen-axis 5) (terminal-provider-pen-axis 6) (terminal-provider-pen-axis 7))) (let ((text1 (read-event nil nil 2)) (text2 (read-event nil nil 2))) (unless (and (= text1 #xe9) (= text2 #x4e2d)) (error \"proto-ui-tpe-input failed: text=%S %S\" text1 text2))) (let ((focus-states nil)) (let ((after-focus-change-function (lambda () (push (frame-parameter frame 'last-focus-update) focus-states)))) (read-event nil nil 2)) (unless (equal focus-states '(nil t)) (error \"proto-ui-tpe-input failed: focus-states=%S\" focus-states))) (unless (equal (progn (read-event nil nil 2) (frame-position frame)) '(31 . 17)) (error \"proto-ui-tpe-input failed: move-position=%S\" (frame-position frame))) (define-key special-event-map [iconify-frame] old-iconify) (define-key special-event-map [make-frame-visible] old-restore) (unless (equal window-states '(make-frame-visible iconify-frame)) (error \"proto-ui-tpe-input failed: window-states=%S\" window-states)) (unless (eq (frame-visible-p frame) t) (error \"proto-ui-tpe-input failed: restored=%S\" (frame-visible-p frame))) (unless (and (= (frame-pixel-width frame) 1200) (= (frame-pixel-height frame) 760) (frame-live-p frame)) (error \"proto-ui-tpe-input failed: size=%Sx%S\" (frame-pixel-width frame) (frame-pixel-height frame))) (unless (and (redisplay t) (terminal-provider-capture-p)) (error \"proto-ui-tpe-input failed: resized-capture\")) (read-event nil nil 0.2) (unless (and (redisplay t) (terminal-provider-capture-p) (terminal-provider-mouse-face-p)) (error \"proto-ui-tpe-input failed: mouse-face=%S\" (terminal-provider-mouse-face-debug))) (let ((motion (track-mouse (read-event nil nil 2)))) (unless (and (eq (car-safe motion) 'mouse-movement) (eq (device-class frame last-event-device) 'mouse)) (error \"proto-ui-tpe-input failed: motion=%S device=%S\" motion last-event-device))) (cancel-timer maximize-timer) (unless (and (member 'maximized maximize-states) (eq (frame-parameter frame 'fullscreen) nil)) (error \"proto-ui-tpe-input failed: maximize-states=%S fullscreen=%S\" maximize-states (frame-parameter frame 'fullscreen))) (let ((close-deadline (+ (float-time) 2))) (while (and (null close-events) (< (float-time) close-deadline)) (read-event nil nil 0.2))) (define-key special-event-map [delete-frame] old-close) (unless (and (= (length close-events) 1) (eq (car-safe (car close-events)) 'delete-frame) (eq (car-safe (cadr (car close-events))) frame) (frame-live-p frame)) (error \"proto-ui-tpe-input failed: close-events=%S frame-live=%S\" close-events (frame-live-p frame))) (select-frame initial-frame) (princ \"proto-ui-tpe-input: pass\") (kill-emacs 0))",
        });
