...
pub fn build(b: *std.Build) void {
    ...
    if (proto_ui_runtime_adapter_target_lib) |adapter_lib| {
        ...
        const run_tpe_frame_smoke = b.addSystemCommand(&[_][]const u8{
            ...
            "--batch",
            "--eval",
            "(let* ((initial-frame (selected-frame)) (terms (terminal-list)) (terminal (car (last terms))) (frame (make-terminal-frame (list (cons 'terminal terminal)))) (second (condition-case nil (make-terminal-frame (list (cons 'terminal terminal))) (error nil)))) (unless (and (frame-live-p frame) (eq (window-system frame) 'proto) (numberp (frame-id frame))) (error \"proto-ui-tpe-frame failed: live=%S ws=%S id=%S\" (frame-live-p frame) (window-system frame) (frame-id frame))) (unless (and (null second) (frame-live-p frame)) (error \"proto-ui-tpe-frame failed: second-frame=%S\" second)) (select-frame frame) (redraw-frame frame) (unless (and (redisplay t) (terminal-provider-capture-p)) (error \"proto-ui-tpe-frame failed: provider capture ack\")) (sit-for 1) (unless (and (redisplay t) (terminal-provider-capture-p)) (error \"proto-ui-tpe-frame failed: owner capture after rejection\")) (princ \"proto-ui-tpe-frame: pass\\n\"))",
        });
