...
| **libpng/libjpeg/libtiff/giflib vendoring**（目标 3.5） | ✅ **已完成 + GUI 落地**：`-Dwith-png/jpeg/tiff`（默认关）把 zig fetch 的三个库源码编为静态库并链入 temacs；`-Dgui` 进一步编译**完整 w32 GUI 后端**（W32_OBJ + fontset/fringe/image + HAVE_NTGUI/HAVE_WINDOW_SYSTEM/TERM_HEADER=w32term.h 等 config + GUI 系统库）。**双后端实测**（本地 + CI `GUI gnu`/`GUI msvc` job，run 32550394083 全绿）：GUI emacs 创建真实 w32 frame，png/jpeg/tiff **真实解码** 1×1 图全部返回 `(1 . 1)`。关键修复：`EMACS_STATIC_IMAGE_LIBS` 下编译掉 image.c 的 WINDOWSNT DLL 懒加载层（`fn_*` 指针），调用直连静态符号（此前全部走 NULL 指针——垃圾数据返回 30×30 fallback 掩盖，真解码即 abort）；libtiff 补 `tif_unix.c`/`tif_hash_set.c`/`HAVE_IEEEFP`；MSVC 侧提供 C 版 gdiplus.h 镜像（SDK 的是 C++-only）+ w32image 桩 | giflib 源不可得（上游无 tarball），保持缺失；GUI 的交互体验（redisplay 性能等）留待后续 |
