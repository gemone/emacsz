...
| **libpng/libjpeg/libtiff/giflib vendoring**（目标 3.5） | ✅ **已完成 + GUI 全链路实测（窗口真实打开）**：`-Dwith-png/jpeg/tiff`（默认关）把 zig fetch 的三个库源码编为静态库并链入 temacs；`-Dgui` 编译**完整 w32 GUI 后端**（W32_OBJ + fontset/fringe/image + config + GUI 系统库），双 ABI 编译+链接全绿（CI `GUI gnu`/`GUI msvc` job）。**交互窗口已验证打开**：`-Q` 启动后 Win32 实测 `title='*' 689×671 at(52,52) visible`——与同机 MSYS2 参考构建的窗口形态**完全一致**（参考为 `'*' 689×671`）；消息泵追踪确认 WM_EMACS_CREATEWINDOW→w32_createwindow(673×610)→握手→持续 WM_PAINT 流。**真实图像解码**：png/jpeg/tiff 1×1 全返回 `(1 . 1)`（`EMACS_STATIC_IMAGE_LIBS` 直连静态符号）。注：zig 构建里 `emacs.exe` 是启动器（spawn temacs.exe 子进程），GUI 窗口归属 temacs pid——早期"窗口 0×0/死锁"结论是**测量错误**（枚举错了进程），已更正 | giflib 源不可得（上游无 tarball），保持缺失。交互体验深度调优（字体渲染、DPI 等）留待日常使用反馈 |
