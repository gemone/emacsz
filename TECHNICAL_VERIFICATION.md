...
| **libpng/libjpeg/libtiff/giflib vendoring**（目标 3.5） | ✅ **已完成 + GUI 编译落地**：`-Dwith-png/jpeg/tiff`（默认关）把 zig fetch 的三个库源码编为静态库并链入 temacs；`-Dgui` 编译**完整 w32 GUI 后端**（W32_OBJ + fontset/fringe/image + HAVE_NTGUI/HAVE_WINDOW_SYSTEM/TERM_HEADER=w32term.h 等 config + GUI 系统库），双 ABI 编译+链接+batch 探针全绿（CI `GUI gnu`/`GUI msvc` job，run 32550394083）。**真实图像解码**已验证：png/jpeg/tiff 解码 1×1 图全部返回 `(1 . 1)`（`EMACS_STATIC_IMAGE_LIBS` 直连静态符号——此前全走 NULL `fn_*` 指针，垃圾数据返回 30×30 fallback 掩盖，真解码即 abort）；libtiff 补 `tif_unix.c`/`tif_hash_set.c`/`HAVE_IEEEFP`；MSVC 侧 C 版 gdiplus.h 镜像 + w32image 桩 | ⚠️ **交互式 GUI 窗口存在启动死锁缺陷（已精确定位，待修）**：`-Q` 交互启动时 Lisp 层 frame 完全就绪（frame-list=1、type=w32、80×36、visibility=t、字体度量 8×16 正常），但 Win32 层窗口 **0×0 不可见**，主线程（GetMessage 等 WM_EMACS_DONE）与 worker 线程（EventPairLow 等待）互等——典型 cross-thread SendMessage 死锁；`frame-pixel-size`（走 Win32 查询路径）同样挂起。GUI 的 batch 模式（CI 探针、ert 全量 582/582）不受影响。需专门一轮 w32 消息泵死锁调试（嫌疑：worker `SendMessage` 与主线程 deferred-msg 互等，参考 w32fns.c `complete_deferred_msg`/`SendMessageTimeout` 路径）。giflib 源不可得，保持缺失 |
| 二进制与上游 MSYS2 构建**一致**（验收 5.1/5.3） | ✅ **功能等价实测**：在本机建立 MSYS2 参考构建（choco msys2 + mingw-w64 gcc 16.2 + emacs-31 分支源内构建，`src/emacs.exe` 全绿产出）。注意：**上游 master 在 mingw 有真实构建回归**（gnulib 快照漂移：sigset_t 双重定义、`<process.h>` 被 `-I../src` 误命中 src/process.h、stdio-consolesafe 的 rpl_free 缺失），逐项绕过不可持续，改用 emacs-31 release 分支一次通过。**等价证据**：① 行为探针 10 项（算术/字符串/排序/base64/md5/sha256/缓冲区/时间格式化/unicode/char-width）两构建**全部一致**（含哈希字节级相同、`char-width ?中`=2）；② 同一批 40 个 ert 套件：Zig 构建 582/582、参考构建 574/574，**各自 0 unexpected**（差 8 个为 31.1→32.0.50 新增测试） | 版本差异（31.1 vs 32.0.50）使逐字节二进制对比无意义（源码本身不同）；行为等价由上述探针+套件对比覆盖。5.2 性能对比（构建时间/运行时）可在需要时用同一参考树补测 |
