...
| **CI 双后端覆盖**（验收 5.3） | 已实现：`.github/workflows/ci.yml` 新增 `build-and-test-msvc` job（GNU 矩阵不动、照旧全量）。本轮关键修复（"假 MSVC"根因）：**后端状态隔离**——① `canonicalConfiguration` 曾对一切 Windows 目标硬编码 `-pc-windows-gnu`，使 msvc 构建的 `system-configuration` 报 gnu 三元组 → 两后端算出**同一个** zeln-abi-hash → 共享一个 zeln-cache，互相加载对方 ABI 的 .zeln（这才是当年 check-zeln 8 个 "Cannot open load file" 的真根因，此前误判为 MSVC 独有）；② dump stamp 单一 `dump.stamp` 使第二后端跳过 dump 复用第一后端的 temacs+pdmp；③ `-p` 安装不刷新 `zig-out/bin/temacs.exe`，msvc 前缀可能装出 gnu 二进制。现 triple/abi-hash/zeln-cache/dump-stamp/-p 全部按 ABI 隔离（gnu=`67173f25`、msvc=`9d6e8583`），dump 改为 `host_can_run_target`（os+arch，Windows 宿主两种 ABI 都能跑）。CI MSVC job 现在构建 + 冒烟 + **全量 ert**（`-Dnative-comp-zig=true` 从一开始就带上，与 GNU job 同构）；.zeln 四步骤暂不进 MSVC job（见下） | MSVC job 的徽章以本轮 push 后的 GitHub run 为准；MSVC 上 check-zeln 仍有 8 个资源加载失败（zeln 替换后的 require 解析差异，**非** ABI 污染——缓存已隔离仍复现），留作后续 |
